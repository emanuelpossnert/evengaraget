...
import { Plus, Edit2, Trash2, DollarSign, Download, X, ChevronDown } from "lucide-react";
  ...
  return (
    <div className="space-y-8">
      ...
      {loading ? (
        ...
        <div className="text-center py-12 text-gray-500">Inga produkter än. Skapa den första!</div>
      ) : (
                  ...
                  {allAddons.map((addon) => (
                    <label key={addon.id} className="flex items-start space-x-2 cursor-pointer p-3 border rounded-lg hover:bg-gray-50">
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-xs text-gray-600">{addon.price} SEK</p>
                      </div>
                    </label>
                  ))}
                </div>
